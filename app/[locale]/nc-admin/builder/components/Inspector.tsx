        ...
        <FieldGroup id="background" title="الخلفية" icon={<Palette className="w-4 h-4" />} expanded={expandedGroups.has('background')} onToggle={() => toggleGroup('background')}>
          ...
          {section.background?.type === 'color' && (
            <div className="flex gap-2">
              <input type="color" value={section.background?.color || '#f8fafc'} onChange={(e) => onUpdate({ background: { ...section.background, type: 'color', color: e.target.value } })} className="w-10 h-10 rounded border cursor-pointer" />
              <input type="text" value={section.background?.color || '#f8fafc'} onChange={(e) => onUpdate({ background: { ...section.background, type: 'color', color: e.target.value } })} className="flex-1 px-3 py-2 text-sm border rounded-lg" dir="ltr" />
            </div>
          )}
        </FieldGroup>
      </div>
